...
def search(cities, max_it, num_ants, decay, c_heur, c_local_phero, c_greed)
  ...
  #compute heuristic cost from heuristic solution
  best[:cost] = cost(best[:vector], cities)

  #initialise the pheromone value
  init_pheromone = 1.0 / (cities.size.to_f * best[:cost])

  #initialise the pheromone for the city matrix
  ...
  max_it.times do |iter|
    ...
    num_ants.times do
      #a candidate solution that has a configuration and a cost
      cand = {}

      #incrementally construct a solution piece-by-piece
